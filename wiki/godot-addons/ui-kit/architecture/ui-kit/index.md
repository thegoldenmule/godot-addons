...
## Summary
**UI Kit** (`addons/ui_kit/`) — generic, game-agnostic UI shell infrastructure for Godot 4.x, extracted from a shipped game. It is a small set of runtime classes: an async stack-FSM navigation router (`UiRouter`), an awaitable per-route base (`UiState`) plus an optional scene-loading subclass (`RouterSceneState`), a layout frame (`UiScreenScaffold`), control-registration factories (`UiReg`), and a semantic UI/navigation automation layer (`UiDriver`). The kit knows *no* game specifics — screens, modes, and flows are all supplied by the consuming project's host. It is **vendored** (copied + committed under the consumer's `addons/ui_kit/` so a fresh clone works offline) and is **updated in place by the `editor_tool_kit` package manager**: ui_kit carries an `[update]` marker in its `plugin.cfg` and ships no update dock of its own, so a consumer that wants ui_kit updates also vendors editor_tool_kit.
...
Self-update: ui_kit carries no update machinery — it opts into the editor_tool_kit package manager with an [update] marker (source/branch/prefix) in plugin.cfg. The version field there on main is still the ship signal — bump it, commit, push. Consumers see it via etk's "Editor Tool Kit" dock (Check all -> Update), which downloads the branch archive and replaces only the addons/ui_kit/ subtree in place (atomic per-file, with rollback). It overwrites + adds but never prunes, so a file removed upstream must be deleted by hand.
...
## Data model
The addon is a handful of `.gd` files under `addons/ui_kit/`, plus a near-empty `plugin.gd` (no update machinery of its own):
...
- **`plugin.gd`** — a near-empty `@tool` `EditorPlugin`: registers **no autoloads** and carries no update dock. Updates are handled by the `editor_tool_kit` package manager via the `[update]` marker in `plugin.cfg`.
